...
function buildNativeFormattingFunc(standardDateProps, extendedSettings, context) {
	...
	return (date) => {
		...
		let s$1 = format.format(marker);
		return postProcess(s$1, date, standardDateProps, extendedSettings, context);
	};
}
...
function buildPublicIdMaps(eventStore) {
	...
	for (let instanceId in instances) {
		...
		const { publicId } = defs[instance.defId];
...
var WindowScrollController = class extends ScrollController {
	...
	setScrollTop(n$1) {
		window.scroll(window.scrollX, n$1);
	}
	setScrollLeft(n$1) {
		window.scroll(n$1, window.scrollY);
	}
	getScrollWidth() {
...
//#endregion
export { BASE_OPTION_DEFAULTS, BaseComponent, BgEvent, Calendar, CustomRenderingStore, DateComponent, DateProfileGenerator, DayCellContainer, DayHeader, DaySeriesModel, DayTableModel, ElementDragging, ElementScrollController, Emitter, EventContainer, EventImpl, Interaction, MoreLinkContainer, NowTimer, PositionCache, RefMap, ScrollController, SegHierarchy, SimpleScrollGrid, Slicer, StandardEvent, ViewContainer, WeekNumberContainer, WindowScrollController, _, addDays, addMs, addWeeks, allowContextMenu, allowSelection, applyMutationToEventStore, applyStyle, buildEntryKey, buildEventApis, buildEventRangeKey, buildNavLinkAttrs, buildSegTimeText, compareNumbers, computeInnerRect, computeRect, config, constrainPoint, createDuration, createEmptyEventStore, createEventInstance, createFormatter, createPlugin, d, diffDates, diffPoints, diffWeeks, disableCursor, elementClosest, elementMatches, enableCursor, eventTupleToStore, formatDayString, formatIsoMonthStr, getClippingParents, getDefaultEventEnd, getElSeg, getEventTargetViaRoot, getRectCenter, getRelevantEvents, getSegAnchorAttrs, getSegMeta, getStickyFooterScrollbar, getStickyHeaderDates, getUniqueDomId, hasCustomDayCellContent, identity, injectStyles, interactionSettingsStore, interactionSettingsToStore, intersectRanges, intersectRects, intersectSpans, isDateSelectionValid, isDateSpansEqual, isInteractionValid, isPropsEqual, mapHash, memoize, parseDragMeta, parseEventDef, pointInsideRect, preventContextMenu, preventSelection, rangeContainsRange, refineEventDef, removeElement, renderFill, renderScrollShim, setRef, sortEventSegs, startOfDay, triggerDateSelect, whenTransitionDone, y };
//# sourceMappingURL=internal-CmEPQwDG.js.map