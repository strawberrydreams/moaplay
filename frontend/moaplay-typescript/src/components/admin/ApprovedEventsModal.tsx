...
export const ApprovedEventsModal: React.FC<ApprovedEventsModalProps> = ({
    ...
    const loadEvents = async (page: number = 1) => {
        try {
            ...
            const result = await getApprovedEvents({ page, limit: 10 });

            // Narrow unknown → known structure
            type ApprovedEventsResp = {
                approvedEvents?: Event[]; // 서버에서 approvedEvents 키로 내려오는 케이스 대응
                items?: Event[];          // 일반화된 items 키 대비
                events?: Event[];         // events 키 대비 (구버전 호환)
                pagination?: Pagination | null;
            };

            const data = result as unknown as ApprovedEventsResp;
            const list: Event[] = Array.isArray(data.approvedEvents)
                ? data.approvedEvents
