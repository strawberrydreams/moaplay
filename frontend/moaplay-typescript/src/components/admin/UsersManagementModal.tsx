...
export const UsersManagementModal: React.FC<UsersManagementModalProps> = ({
    ...
    // 사용자 역할 변경 (추후 구현)
    const handleRoleChange = async (_userId: number, currentRole: string) => {
    ...
    const handleDeleteUser = async () => {
        const ok = window.confirm('정말 이 사용자를 삭제할까요? 이 작업은 되돌릴 수 없어요.');
        if (!ok) return;

        const password = window.prompt('본인 확인용 비밀번호를 입력하세요') ?? '';
        if (!password) {
            alert('비밀번호가 필요합니다.');
            return;
        }
