...
interface UseAdminUsersReturn {
  ...
  // 액션 함수들
  loadUsers: (page?: number, role?: 'user' | 'organizer' | 'admin') => Promise<void>;
  updateUser: (userId: number, userData: UpdateUserRequest) => Promise<void>;
  deleteUser: (userId: number) => Promise<void>;
  refreshUsers: () => Promise<void>;
  setRoleFilter: (role: 'user' | 'organizer' | 'admin' | undefined) => void;
}
...
export const useAdminUsers = (): UseAdminUsersReturn => {
  ...
  // 필터 상태
  const [currentRole, setCurrentRole] = useState<'user' | 'organizer' | 'admin' | undefined>(undefined);
   ...
   * @param role 역할 필터
   */
  const loadUsers = useCallback(async (page: number = 1, role?: 'user' | 'organizer' | 'admin') => {
    try {
      setLoading(true);
      setError(null);
      
      const result = await AdminService.getUsers(page, 20, role);
   ...
   * @param role 필터할 역할
   */
  const setRoleFilter = useCallback((role: 'user' | 'organizer' | 'admin' | undefined) => {
    setCurrentRole(role);
    // 필터 변경 시 첫 페이지로 이동
    loadUsers(1, role);
  }, [loadUsers]);
